...
def calc_speed(frames, video_path, result_yolo_path, speed_file):
    result = []
...
def MakeDataset(
    n_data, n_class, speed_file, label_file, images_folder, h_size=256, w_size=256
):
    speed_file = open(speed_file, "r")
    speed_data = speed_file.read()
    speed_lines = speed_data.split("\n")
    speed_lines = speed_lines[:-1]
    input_data = []
    ...
    for i in range(n_data):
        ...
        image_path = f"{images_folder}{input_data[i][1]}.jpg"
