...
# get
async def get_enrichments(date, animalId, db: AsyncSession):
    stmt = (
        select(
            Enrichment.id,
            Enrichment.enrichment,
        )
        .where(and_(Enrichment.date == date, Enrichment.animalId == animalId))
...
async def create_enrichment(
    ...
    new_enrichment = Enrichment(
        animalId=enrichment_create.animalId,
