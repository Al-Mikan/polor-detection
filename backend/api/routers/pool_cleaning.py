...
async def create_pool_cleaning(
    meal: schema.PoolCleaningCreate, db: AsyncSession = Depends(get_db)
):
...
async def update_pool_cleaning(
    id: int, new_enrichment: schema.PoolCleaningBase, db: AsyncSession = Depends(get_db)
):
