...
@router.get("/api/water/{polorId}", response_model=List[schema.Water])
async def get_water(polorId: int,date: str = Query(...), db: AsyncSession = Depends(get_db)):
        requested_date = datetime.strptime(date, "%Y-%m-%d").date()
        return await cruds.get_water(requested_date,polorId,db)
...
async def update_water(id: int, new_el: schema.WaterBase ,db: AsyncSession = Depends(get_db)):
