...
async def create_expropriation(
    meal: schema.ExpropriationCreate, db: AsyncSession = Depends(get_db)
):
    return await crud.create_expropriation(db, meal)


@router.put(
    "/api/expropriation/{id}", response_model=schema.ExpropriationCreateResponse
)
async def update_expropriation(
    id: int, new_event: schema.ExpropriationBase, db: AsyncSession = Depends(get_db)
):
