...
@router.get("/api/wakeupTime/{polorId}", response_model=List[schema.WakeUpTime])
async def get_wakeup_time(polorId: int,date: str = Query(...), db: AsyncSession = Depends(get_db)):
        requested_date = datetime.strptime(date, "%Y-%m-%d").date()
        return await cruds.get_wakeup_time(requested_date,polorId,db)

@router.post("/api/wakeupTime", response_model=schema.WakeUpTimeCreateResponse)
async def create_wakeup_time(elm: schema.WakeUpTimeCreate,db: AsyncSession = Depends(get_db)):
    return await cruds.create_wakeup_time(db,elm)

@router.put("/api/wakeupTime/{id}", response_model=schema.WakeUpTimeCreateResponse)
async def update_wakeup_time(id: int, new_el: schema.WakeUpTimeBase ,db: AsyncSession = Depends(get_db)):
