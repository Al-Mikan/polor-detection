...
@router.get("/api/training/{polorId}", response_model=List[schema.Training])
async def get_training(polorId: int, date: str = Query(...), db: AsyncSession = Depends(get_db)):
        requested_date = datetime.strptime(date, "%Y-%m-%d").date()
        return await crud.get_training(requested_date,polorId,db)


@router.post("/api/training", response_model=schema.TrainingCreateResponse)
async def create_training(temp: schema.TrainingCreate,db: AsyncSession = Depends(get_db)):
    return await crud.create_training(db,temp)

@router.put("/api/training/{id}", response_model=schema.TrainingCreateResponse)
async def update_training(id: int, new_temp: schema.TrainingBase ,db: AsyncSession = Depends(get_db)):
