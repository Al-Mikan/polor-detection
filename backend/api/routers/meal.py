...
async def get_meals(polorId: int, date: str = Query(...), db: AsyncSession = Depends(get_db)):
        requested_date = datetime.strptime(date, "%Y-%m-%d").date()
        return await meal_crud.get_meals(requested_date,polorId,db)
...
@router.put("/meals/{id}", response_model=schema.MealCreateResponse)
async def update_meal(id: int, new_meal: schema.MealBase ,db: AsyncSession = Depends(get_db)):
